...
**Ноутбук в повседневной работе не нужен вообще.** Доступ заводится на
сервере (`deploy/mkuser.mjs`), участники появляются через панель, лендинг
правится там же, история — в `backups/` на сервере. Ноутбук нужен только
...
Пароль **nginx** на панели. Это первый и главный слой: без него `/admin/`
отдал бы `users.json` и карточки участников любому, а вход внутри панели
клиентский — его обходят через инструменты разработчика.
...
Вход **в панель** — второй слой, свои логины и роли. Заводится здесь же, на
сервере:

```bash
cd /srv/camp && node deploy/mkuser.mjs admin
```

Спросит пароль дважды, не отображая. Роль по умолчанию `admin`; бывают
`admin`, `content`, `lead`, `desk`:

```bash
cd /srv/camp && node deploy/mkuser.mjs anna desk "Аня"
```

Проверить пароль, ничего не меняя, и посмотреть список:

```bash
cd /srv/camp && node deploy/mkuser.mjs admin --check
cd /srv/camp && node deploy/mkuser.mjs --list
```

Почему отдельная команда, а не кнопка в панели: панель **намеренно** не даёт
завести доступ самому себе. На публичном адресе «стать главным» было бы
захватом панели тем, кто открыл её первым. Поэтому первый пользователь
появляется там, где нужен доступ к серверу, а дальше остальных заводит
панель.

Хэш считается ровно как в `admin.html` — PBKDF2-SHA256, 120 000 итераций,
соль как строка. Совпадение закреплено в `mkuser.mjs --selftest` вектором,
снятым с настоящего `crypto.subtle`: разойдись параметры, панель просто не
приняла бы пароль, и понять это было бы неоткуда.
...
20 списков участников. Снимок делается на каждое сохранение. `users.json` и
`integrations.json` не снимаются намеренно — множить по папке копии хэшей и
...
Новый человек в панель:

```bash
cd /srv/camp && node deploy/mkuser.mjs ivan lead "Иван"
```
