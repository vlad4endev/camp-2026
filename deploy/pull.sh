...
#
# .mjs, .js и .css берём маской, а не списком имён: server.mjs импортирует
...
#
# .css попал сюда как раз этим путём: camp-ui.css появился рядом с
# camp-ui.js, маска ловила только .js — и палитра трёх панелей осталась
# бы на ноутбуке. Сервер бы не упал, selftest бы прошёл, а штаб и стойка
# открылись бы без единого цвета. Ровно та поломка, ради которой здесь
# маски, а не имена.
...
install -m644 -o camp -g camp *.mjs *.js *.css manifest.webmanifest \
